    ...
    <div class="p-6 border-b border-gray-200 flex justify-between items-center">
        <div>
            <h2 class="text-lg font-semibold text-gray-800">
                Riwayat: {{ $stok->obat->nama ?? '-' }}
            </h2>
            <p class="text-sm text-gray-500">Gudang: {{ $stok->gudang->nama_gudang ?? '-' }} — Stok saat ini: {{ number_format($stok->jumlah, 2) }} {{ $stok->obat->satuan ?? '' }}</p>
        </div>
        <a href="{{ route('obat.stok.index') }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Kembali</a>
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b">
                    ...
                    <td class="px-3 py-2 text-right text-green-600">+{{ number_format($log->jumlah_baru, 2) }}</td>
                    <td class="px-3 py-2 text-right font-semibold text-gray-900">{{ number_format($log->total, 2) }}</td>
