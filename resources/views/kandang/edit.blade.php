...
@section('title', 'Edit Kandang - SIKAP')
@section('page-title', 'Edit Kandang')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Edit Kandang: {{ $kandang->nama_kandang }}</h2>
        </div>

        <form action="{{ route('kandang.update', $kandang) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <x-input-label for="kode_kandang" :value="'Kode Kandang'" />
                    <x-text-input id="kode_kandang" name="kode_kandang" class="block mt-1 w-full" :value="old('kode_kandang', $kandang->kode_kandang)" required />
                ...
                <div>
                    ...
                    <select id="gudang_id" name="gudang_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        ...
                        @foreach($gudangs as $g)
                            <option value="{{ $g->id }}" {{ old('gudang_id', $kandang->gudang_id) == $g->id ? 'selected' : '' }}>{{ $g->nama_gudang }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="nama_kandang" name="nama_kandang" class="block mt-1 w-full" :value="old('nama_kandang', $kandang->nama_kandang)" required />
            ...
            <div>
                ...
                <x-text-input id="alamat_jalan" name="alamat_jalan" class="block mt-1 w-full" :value="old('alamat_jalan', $kandang->alamat_jalan)" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <x-text-input id="desa_kelurahan" name="desa_kelurahan" class="block mt-1 w-full" :value="old('desa_kelurahan', $kandang->desa_kelurahan)" required />
                    <x-input-error :messages="$errors->get('desa_kelurahan')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="kecamatan" :value="'Kecamatan'" />
                    <x-text-input id="kecamatan" name="kecamatan" class="block mt-1 w-full" :value="old('kecamatan', $kandang->kecamatan)" required />
                    <x-input-error :messages="$errors->get('kecamatan')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="kabupaten_kota" :value="'Kabupaten/Kota'" />
                    <x-text-input id="kabupaten_kota" name="kabupaten_kota" class="block mt-1 w-full" :value="old('kabupaten_kota', $kandang->kabupaten_kota)" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="provinsi" name="provinsi" class="block mt-1 w-full" :value="old('provinsi', $kandang->provinsi)" required />
                    <x-input-error :messages="$errors->get('provinsi')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="kode_pos" :value="'Kode Pos'" />
                    <x-text-input id="kode_pos" name="kode_pos" class="block mt-1 w-full" :value="old('kode_pos', $kandang->kode_pos)" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <x-text-input id="kapasitas" name="kapasitas" type="number" class="block mt-1 w-full" :value="old('kapasitas', $kandang->kapasitas)" required min="1" />
                ...
                <div>
                    ...
                    <select id="tipe_kandang" name="tipe_kandang" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" required>
                        ...
                        <option value="baterai" {{ old('tipe_kandang', $kandang->tipe_kandang) == 'baterai' ? 'selected' : '' }}>Baterai</option>
                        <option value="postal" {{ old('tipe_kandang', $kandang->tipe_kandang) == 'postal' ? 'selected' : '' }}>Postal</option>
                        <option value="closed_house" {{ old('tipe_kandang', $kandang->tipe_kandang) == 'closed_house' ? 'selected' : '' }}>Closed House</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" required>
                        ...
                        <option value="renovasi" {{ old('status', $kandang->status) == 'renovasi' ? 'selected' : '' }}>Renovasi</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="latitude" name="latitude" type="number" step="any" class="block mt-1 w-full" :value="old('latitude', $kandang->latitude)" />
                    <x-input-error :messages="$errors->get('latitude')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="longitude" :value="'Longitude (opsional)'" />
                    <x-text-input id="longitude" name="longitude" type="number" step="any" class="block mt-1 w-full" :value="old('longitude', $kandang->longitude)" />
            ...
            <div>
                ...
                @if($kandang->foto)
                    <div class="mt-1 mb-2">
                        <img src="{{ asset('storage/' . $kandang->foto) }}" alt="Foto kandang" class="w-32 h-32 object-cover rounded-lg border">
                    </div>
                @endif
