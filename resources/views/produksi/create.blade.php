...
@section('title', 'Tambah Produksi Telur - SIKAP')
@section('page-title', 'Tambah Produksi Telur')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Form Input Produksi Telur</h2>
        </div>

        <form action="{{ route('produksi.store') }}" method="POST" class="p-6 space-y-4" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <select id="kandang_id" name="kandang_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" required>
                    ...
                    <option value="{{ $k->id }}" {{ old('kandang_id') == $k->id ? 'selected' : '' }}>{{ $k->nama_kandang }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="karpet" name="karpet" type="number" class="block mt-1 w-full" :value="old('karpet')" placeholder="0" required min="0" oninput="hitungButir()" />
                ...
                <div>
                    ...
                    <x-text-input id="sisa" name="sisa" type="number" class="block mt-1 w-full" :value="old('sisa')" placeholder="0" min="0" max="29" oninput="hitungButir()" />
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="butir_karpet" type="number" class="block mt-1 w-full bg-gray-100" :value="old('butir_karpet')" readonly />
                    <p class="text-xs text-gray-500 mt-1">Karpet &times; 30 butir</p>
                </div>
                <div>
                    <x-input-label for="jumlah_butir" :value="'Total Butir (auto)'" />
                    <x-text-input id="jumlah_butir" name="jumlah_butir" type="number" class="block mt-1 w-full bg-gray-100" :value="old('jumlah_butir')" readonly required />
            ...
            <div>
                ...
                <label class="flex items-center justify-center gap-2 mt-1 px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-indigo-400 bg-gray-50 text-sm text-gray-600">
