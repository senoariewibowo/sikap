...
@section('title', 'Input Transaksi Eceran - SIKAP')
@section('page-title', 'Input Transaksi Eceran')
@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('eceran.transaksi.store') }}">
            @csrf
            <div class="space-y-4">
                <div>
                    <x-input-label :value="'Tanggal'" />
                    <input type="date" name="tanggal" value="{{ old('tanggal', now()->format('Y-m-d')) }}" class="block mt-1 w-full border-gray-300 rounded-md text-sm" required>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="satuan" value="per_butir" {{ old('satuan', 'per_butir') == 'per_butir' ? 'checked' : '' }} onchange="toggleSatuan()" class="text-green-600">
                            <span class="text-sm">Per Butir</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="satuan" value="per_kg" {{ old('satuan') == 'per_kg' ? 'checked' : '' }} onchange="toggleSatuan()" class="text-green-600">
                            <span class="text-sm">Per Kg</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="satuan" value="per_karpet" {{ old('satuan') == 'per_karpet' ? 'checked' : '' }} onchange="toggleSatuan()" class="text-green-600">
                ...
                <div id="section_kg" class="hidden">
                    ...
                    <input type="number" step="0.01" name="berat_kg" id="berat_kg" value="{{ old('berat_kg') }}" min="0.01" class="block mt-1 w-full border-gray-300 rounded-md text-sm">
                </div>

                <div id="section_karpet" class="hidden">
                    <x-input-label :value="'Karpet (tray)'" />
                    <input type="number" name="karpet" id="karpet" value="{{ old('karpet') }}" min="1" class="block mt-1 w-full border-gray-300 rounded-md text-sm">
                    <p class="text-xs text-gray-400 mt-1">1 karpet = 30 butir &middot; otomatis terisi</p>
                </div>

                <div>
                    <x-input-label :value="'Jumlah Butir'" />
                    <input type="number" name="jumlah_butir" id="jumlah_butir" value="{{ old('jumlah_butir') }}" min="1" max="{{ $pool }}" class="block mt-1 w-full border-gray-300 rounded-md text-sm" required>
                    <p id="stok_warning" class="hidden text-xs text-red-500 mt-1">Jumlah melebihi stok eceran tersedia!</p>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="2" class="block mt-1 w-full border-gray-300 rounded-md text-sm" placeholder="Misal: Pembeli walk-in...">{{ old('keterangan') }}</textarea>
                </div>
                <div class="flex space-x-2">
                    <button type="submit" class="inline-flex items-center gap-1 px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 font-semibold"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"></svg>Simpan Transaksi</button>
...
(function(){
    ...
    var stokWarning = document.getElementById('stok_warning');
    var pool = {{ $pool }};
    ...
    function toggleSatuan(){
        ...
        if(isKg){
            harga.value = hargaKg.toLocaleString('id-ID');
            beratKg.setAttribute('required', 'required');
            karpet.removeAttribute('required');
        } else if(isKarpet){
            harga.value = hargaKarpet.toLocaleString('id-ID');
            ...
            harga.value = hargaButir.toLocaleString('id-ID');
            beratKg.removeAttribute('required');
            karpet.removeAttribute('required');
        }
        updateTotal();
    ...
    jumlahButir.addEventListener('input', function(){ validateStok(); updateTotal(); });
    ...
    function validateStok(){
        var val = parseInt(jumlahButir.value) || 0;
        var over = pool > 0 && val > pool;
        jumlahButir.classList.toggle('border-red-500', over);
        jumlahButir.classList.toggle('focus:ring-red-500', over);
        stokWarning.classList.toggle('hidden', !over);
    }
