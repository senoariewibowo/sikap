...
@section('title', 'Edit Telur Keluar - SIKAP')
@section('page-title', 'Edit Telur Keluar')
@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b"><h2 class="text-lg font-semibold text-gray-800">Edit Data Telur Keluar</h2></div>
        <form action="{{ route('telur.keluar.update', $stokTelur) }}" method="POST" class="p-6 space-y-4" id="formKeluar">@csrf @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <x-input-label :value="'Tanggal'" />
                    <x-text-input name="tanggal" type="date" class="block mt-1 w-full" :value="old('tanggal', $stokTelur->tanggal->format('Y-m-d'))" required />
                </div>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mb-2">Gudang: {{ $stokTelur->gudang->nama_gudang ?? '-' }}</p>
                ...
                @forelse($allPetis as $gudangNama => $petis)
                <div class="mb-4 border rounded-lg overflow-hidden gudang-group">
                    ...
                    <div class="p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-96 overflow-y-auto peti-list">
                        @foreach($petis as $p)
                        <label class="peti-label flex items-start gap-2 p-2 border rounded hover:bg-gray-50 cursor-pointer transition {{ $p->selected ?? false ? 'bg-indigo-50 border-indigo-300' : '' }}">
                            <input type="checkbox" name="peti_ids[]" value="{{ $p->id }}" class="peti-checkbox mt-1" data-butir="{{ $p->butir }}" data-berat="{{ $p->berat }}" data-karpet="{{ $p->karpet }}" data-kandang="{{ $p->kandang_nama ?? '-' }}" data-kode_peti="{{ $p->kode_peti ?? '-' }}" data-tanggal="{{ $p->tgl_sortir }}" {{ $p->selected ?? false ? 'checked' : '' }}>
                ...
                @empty
                <div class="p-4 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">Tidak ada peti sortasi yang tersedia.</div>
                @endforelse
            ...
            <div>
                <x-input-label :value="'No. Surat Jalan'" />
                <x-text-input class="block mt-1 w-full bg-gray-100 cursor-not-allowed" :value="$stokTelur->no_referensi ?: '-'" disabled />
            </div>

            <div>
                <x-input-label :value="'Keterangan'" />
                <textarea name="keterangan" rows="2" class="block mt-1 w-full border-gray-300 rounded-md text-sm">{{ old('keterangan', $stokTelur->keterangan) }}</textarea>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t">
                ...
                <x-primary-button type="submit" id="btnSimpan">Simpan</x-primary-button>
            </div>
        </form>
    </div>
</div>
...
(function(){
    ...
    function filterPeti(){
        ...
        gudangGroups.forEach(function(group){
            var visible = group.querySelectorAll('.peti-label').length > group.querySelectorAll('.peti-label[style*="display: none"]').length;
...
function driverSelect() {
    ...
    return {
        drivers: drivers,
        search: '{{ old('driver', $stokTelur->driver) }}',
        selected: '{{ old('driver', $stokTelur->driver) }}',
        open: false,
        get filteredDrivers() {
