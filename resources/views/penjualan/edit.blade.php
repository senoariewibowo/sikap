...
@section('title', 'Edit Penjualan - SIKAP')
@section('page-title', 'Edit Transaksi Penjualan')

@section('content')
<div class="max-w-4xl mx-auto" x-data="customerSelect()">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b"><h2 class="text-lg font-semibold text-gray-800">Edit Transaksi Penjualan</h2></div>
        <form action="{{ route('penjualan.update', $transaksi) }}" method="POST" class="p-6 space-y-4" id="formPenjualan">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input name="tanggal" type="date" class="block mt-1 w-full" value="{{ old('tanggal', $transaksi->tanggal->format('Y-m-d')) }}" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <x-text-input id="harga_per_satuan" name="harga_per_satuan" type="text" data-type="rupiah" autocomplete="off" class="block mt-1 w-full" :value="old('harga_per_satuan', number_format($transaksi->harga_per_satuan, 0, ',', '.'))" required />
                ...
                <div>
                    ...
                    <x-text-input id="total_harga" type="text" class="block mt-1 w-full bg-gray-100" value="Rp {{ number_format($transaksi->total_harga, 0, ',', '.') }}" readonly />
                </div>
                <div>
                    <x-input-label :value="'Jumlah Peti'" />
                    <x-text-input id="jumlah_satuan" type="number" class="block mt-1 w-full bg-gray-100" :value="old('jumlah_satuan', $transaksi->jumlah_satuan)" readonly />
                </div>
            </div>
                        ...
                        <div class="p-3 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <label class="detail-card flex items-start gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition {{ $errors->has('detail_qty.'.$d->id) ? 'border-red-300 bg-red-50' : '' }} {{ $d->selected ? 'bg-indigo-50 border-indigo-300' : '' }}">
                                        <input type="checkbox" class="detail-checkbox mt-1.5" name="detail_qty[{{ $d->id }}]" value="1" data-id="{{ $d->id }}" data-butir="{{ $d->sisa }}" data-berat="{{ $d->berat_kg }}" data-kode="{{ $d->sortasiTelurDetail->kode_peti ?? '-' }}" data-sj="{{ $sj->no_referensi ?: '-' }}" data-gudang="{{ $gudangNama }}" {{ $d->selected ? 'checked' : '' }}>
                                        <div class="flex-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="status_pembayaran" name="status_pembayaran" class="block mt-1 w-full border-gray-300 rounded-md text-sm" required>
                        <option value="belum_lunas" {{ old('status_pembayaran', $transaksi->status_pembayaran)=='belum_lunas'?'selected':'' }}>Belum Lunas</option>
            ...
            <div id="dpField" class="{{ old('status_pembayaran', $transaksi->status_pembayaran)=='lunas'?'hidden':'' }}">
                <x-input-label :value="'Nominal DP / Uang Muka (Rp)'" />
                <x-text-input name="dp" type="text" data-type="rupiah" autocomplete="off" class="block mt-1 w-full" :value="old('dp', number_format($transaksi->dp, 0, ',', '.'))" />
            </div>

            <div>
                <x-input-label :value="'Catatan Pembayaran'" />
                <textarea name="catatan_pembayaran" rows="2" class="block mt-1 w-full border-gray-300 rounded-md text-sm" placeholder="Contoh: Bank BCA 123456789 a/n Driver X">{{ old('catatan_pembayaran', $transaksi->catatan_pembayaran) }}</textarea>
            </div>

            <div class="flex justify-end space-x-3 pt-4 border-t">
                <a href="{{ route('penjualan.index') }}" class="px-4 py-2 text-sm text-gray-700 bg-white border rounded-lg">Batal</a>
                <x-primary-button type="submit" id="btnSimpan" disabled>Perbarui Transaksi</x-primary-button>
            </div>
        </form>
    </div>
</div>
...
function customerSelect(){
    ...
    var selectedCustomer = @json($transaksi->customer);
    return {
        customers: customers,
        customerSearch: selectedCustomer ? selectedCustomer.nama_customer + (selectedCustomer.alamat ? ' (' + selectedCustomer.alamat + ')' : '') : '',
