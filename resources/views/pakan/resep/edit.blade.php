...
@section('title', 'Edit Resep Pakan - SIKAP')
@section('page-title', 'Edit Resep Pakan')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Form Edit Resep Pakan</h2>
        </div>

        <form action="{{ route('pakan.resep.update', $resep) }}" method="POST" class="p-6 space-y-4" id="resepForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="pakan_id" name="pakan_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                        ...
                        @foreach($pakans as $p)
                        <option value="{{ $p->id }}" {{ old('pakan_id', $resep->pakan_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }} ({{ $p->satuan }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="nama_resep" name="nama_resep" class="block mt-1 w-full" :value="old('nama_resep', $resep->nama_resep)" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="flex items-center">
                    <input id="is_default" name="is_default" type="checkbox" value="1" {{ old('is_default', $resep->is_default) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-sm" required>
                        ...
                        <option value="nonaktif" {{ old('status', $resep->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                    <x-input-error :messages="$errors->get('status')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="keterangan" :value="'Keterangan (opsional)'" />
                    <x-text-input id="keterangan" name="keterangan" class="block mt-1 w-full" :value="old('keterangan', $resep->keterangan)" />
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500" id="detailsTable">
                        ...
                        <tbody id="detailsBody">
                            @php
                                $details = old('details') ?: $resep->details->map(fn($d) => [
